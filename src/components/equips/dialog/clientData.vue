<template>
  <div class="formatCard pa-5 ">
      <v-dialog persistent v-model="whatsappDialog.active" width="400px">
      <whatsappView :propUrl="whatsappDialog.url" @method="methodsWp" />
    </v-dialog>
    <v-row>
      <v-spacer></v-spacer>
...
<script>
import whatsappView from "@/components/equips/dialog/whatsappView";
export default {
  components: {
    whatsappView
  },
  ...
  data() {
    return {
      form: { name: "", contact: "" },
      whatsappDialog:{active:false, url:""},
      sms: "",
    };
  },
  computed: {},
    ...
    sendForm(aux) {
      ...
      if (!(this.form.name.length > 4 && this.form.contact.length > 4)) {
       ...
       var url =
              ...
              this.form.contact +
              ". Estoy interesado en el siguiente equipo: " +
              aux.name;
            window.open(url, "_blank");
      this.$emit("method", "close");
     // this.whatsappDialog.url=url
     // this.whatsappDialog.active=true
            
    },
    methodsWp(){
      this.whatsappDialog.active=false
    }
  },
};
</script>
